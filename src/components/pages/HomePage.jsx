...
const HomePage = () => {
	// 🍀 HOOKS & VARIABLES:
	const { crypto, cryptoLoading, cryptoError } = useSelector(
		({ crypto }) => crypto
	);
				...
				<table className='w-full'>
					<thead className='bg-gray-200 bottom-b-2 border-gray-200'>
						<tr className='divide-x divide-gray-300 text-slate-600'>
							<th
								className='p-3 text-sm font-semibold tracking-wide text-left uppercase cursor-pointer transition-all hover:bg-indigo-400 hover:text-white'
								onClick={() => setSort('id')}
							>
								Coin
							</th>
							...
							<th
								className='p-3 text-sm font-semibold tracking-wide text-left uppercase cursor-pointer transition-all hover:bg-indigo-400 hover:text-white'
								onClick={() =>
									setSort('price_change_percentage_24h')
								}
							>
								24h
							</th>
